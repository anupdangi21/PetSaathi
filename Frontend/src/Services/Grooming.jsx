...
import { Heart, MapPin, Calendar, Info, BadgeAlert, Package, PackageOpen } from 'lucide-react';
...
const Grooming = () => {
  ...
  const [showGroom, setShowGroom] = useState([]);
            ...
            <div key={pet._id} className="bg-zinc-50 rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => withAuth(() => handleGrooming(pet))()}
        ...
        {selectedPet && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            ...
            >
              ...
              <div className="p-6">
                <div className='flex flex'>
                  <h2 className="text-lg font-bold mb-4">From: </h2>
                  <p className=' text-lg mt-0.5 ml-2'>{selectedPet.organizationname}</p>
                </div>
                <div className='flex flex'>
                  <h2 className="text-lg font-bold mb-4">Contact vendor:</h2>
                  <p className='mt-1 ml-2'>98*******</p>
                </div>
                
                <div className="flex mb-2">
                  <PackageOpen size={24} className="" />
                  <span className='font-bold text-lg flex ml-1'> Included Offerings:</span>
                  <p className='flex flex-1 ml-1 mt-0.5 text-lg'>{selectedPet.includedOfferings}</p>
                </div>
                <div className="col-span-2 mb-6 flex">
                  <span className='text-xl font-semibold'> Description:</span>
                  <p className=' ml-1 mt-0.5'>{selectedPet.description}</p>
                </div>
                <div className='flex flex mb-6'>
                  <h3 className="text-xl font-semibold text-gray-800">Status:</h3>
                  <span className="text-green-500">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => withAuth(() => handleGrooming(selectedPet))()}
...
{showConfirmation && bookingPet && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div ref={modalRef} className="bg-white shadow-md rounded-lg p-6 min-h-[40vh] w-full max-w-[800px]">
              <GroomingReservation pet={bookingPet} onClose={closeModal} />
            </div>
          </div>
        )}
      </main>
