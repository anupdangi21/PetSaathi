...
import EsewaIntegrationHostel from "../Payment/EsewaIntegrationhostel.jsx"
...
const HostelReservation = () => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('hosteldetails', JSON.stringify(hosteldetails));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        localStorage.removeItem('hosteldetails'); // Clear reservation details on success
      }
    } catch (error) {
      console.error(error);
      Swal.fire({
  ...
  const handleAccommodationChange = (e) => {
    ...
    setPrice(parseFloat(accommodation.price));
  };
  const calculatedAmount = price && days ? price * parseInt(days) : 0;
  return (
    <div className='w-full max-w-[800px] mx-auto'>
      
      {initiateEsewa && calculatedAmount > 0 && (
  <EsewaIntegrationHostel amount={calculatedAmount} />
)}
