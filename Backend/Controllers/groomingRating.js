...
const updateGroomingRating = async(req,res)=>{
    try {
        // console.log(req.body,"aakai ho?")
        const { email, username, vendoremail, organizationname } = req.body;
        const petId = req.params.id;
        const pet = await GroomingRating.findById(petId)
        ...
        const mailOptionsUser = {
         ...
         const mailOptionsVendor = {
            ...
            to: vendoremail, // vendor's email
            subject: "Pet Grooming Service Rating Response",
            text: `Hello ${organizationname}, you have marked the rating response from ${username} to read. 
